...
namespace QuanLySinhVien
{
    public partial class f_EditStudent : Form
    {
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            string mssvText = txtMSSV.Text.Trim();

            // 1. CHỦ ĐỘNG KIỂM TRA RÀNG BUỘC: Kiểm tra xem sinh viên đã đăng ký môn học nào chưa
            try
            {
                db.openConnection();
                // Biến ép kiểu int vì MSSV trong database của ní là kiểu INT số nguyên
                int mssvInt = Convert.ToInt32(mssvText);

                string checkQuery = "SELECT COUNT(*) FROM DKMH WHERE MSSV = @mssv";
                SqlCommand cmdCheck = new SqlCommand(checkQuery, db.conn);
                cmdCheck.Parameters.AddWithValue("@mssv", mssvInt);

                int count = Convert.ToInt32(cmdCheck.ExecuteScalar());
                db.closeConnection(); // Đóng kết nối ngay sau khi check xong

                if (count > 0)
                {
                    // Nếu có dữ liệu trong bảng DKMH, hiển thị thông báo chặn đứng hành vi xóa
                    MessageBox.Show($"Không thể xóa! Sinh viên mã số [{mssvText}] hiện đang đăng ký học môn học trên hệ thống.\nVui lòng hủy toàn bộ môn học đã đăng ký của sinh viên này trước khi xóa dữ liệu gốc.",
                                    "Cảnh báo ràng buộc", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return; // Ngắt lệnh, không thực thi đoạn chạy DELETE bên dưới nữa
                }
            }
            catch (Exception ex)
            {
                MessageBox.Show("Lỗi kiểm tra ràng buộc dữ liệu: " + ex.Message, "Lỗi hệ thống", MessageBoxButtons.OK, MessageBoxIcon.Error);
                db.closeConnection();
                return;
            }

            // 2. TIẾN HÀNH XÓA: Nếu vượt qua bộ lọc check ở trên (count == 0) thì cho phép xác nhận xóa
            DialogResult confirm = MessageBox.Show("Bạn có chắc chắn muốn xóa sinh viên mã " + mssvText + " không?",
                                                   "Xác nhận xóa hẳn", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
            if (confirm == DialogResult.Yes)
            {
                try
                {
                    ...
                    SqlCommand cmd = new SqlCommand(query, db.conn);
                    cmd.Parameters.AddWithValue("@mssv", Convert.ToInt32(mssvText));

                    int result = cmd.ExecuteNonQuery();
                    if (result > 0)
                    {
                        MessageBox.Show("Xóa dữ liệu sinh viên thành công khỏi hệ thống!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        btnRefresh_Click(sender, e); // Làm mới lại form và nạp lại danh sách
                    }
                }
                catch (Exception ex)
                {
                    MessageBox.Show("Lỗi phát sinh trong quá trình xóa dữ liệu: " + ex.Message, "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
                finally
                {
                    db.closeConnection();
